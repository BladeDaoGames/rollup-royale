...
class EventScanner:
    ...
    def __init__(self, w3: Web3, contract: Contract, state: EventScannerState, 
                events: List, filters: Dict[str, Any],
    ...
    def scan(self, start_block, end_block, start_chunk_size=20, progress_callback=Optional[Callable]) -> Tuple[
        ...
        while current_block <= end_block:
            ...
            estimated_end_block = min(estimated_end_block, end_block)
            logger.debug(
                f"Scanning Event for blocks: {current_block} - {estimated_end_block}, chunk size {chunk_size}, last chunk scan took {last_scan_duration}, last logs found {last_logs_found}"
            )
...
class JSONifiedState(EventScannerState):
        ...
        def __init__(self, filename:str):
            self.state = None
            self.fname = filename #"test-state.json"
        ...
        def process_event(self, block_when: datetime.datetime, event: AttributeDict) -> str:
            ...
            args = event["args"]
            eventObject = dict(args)
            #print(args)
            eventObject.update(timestamp=block_when.isoformat())
            
            # eventObject = {
            #     "_roomId": args._roomId,
            #     "_creator": args._creator,
            ...
            self.state["blocks"][block_number][txhash][log_index] = eventObject
...
def getEventData(eventName:str, rpc_url:str, contractAddress:str, 
                abi:list, contractCreationBlock:int):
    try:
        ...
        state = JSONifiedState(str(eventName)+".json")
        state.restore()
        ...
        scanner = EventScanner(
            ...
            contract=contract,
            state=state,
            events=[contract.events[eventName]],
        ...
        start_block = max(state.get_last_scanned_block() - chain_reorg_safety_blocks, contractCreationBlock)
        end_block = scanner.get_suggested_scan_end_block()
        blocks_to_scan = end_block - start_block

        print(f"Scanning event: {eventName} from blocks {start_block} - {end_block}")
        ...
        print(f"Scanned total {len(result)} '{eventName}', in {duration} seconds, total {total_chunks_scanned} chunk scans performed")
        return True
    except Exception as e:
        print(e)
        return False
...
if __name__ == "__main__":
    ...
    ROYALE_ADDRESS = os.environ.get("ROYALE_ADDRESS")
    CONTRACT_CREATIONBLOCK = os.environ.get("ROYALE_CREATEBLOCK")
    ROOT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    # Reduced ERC-20 ABI, only Transfer event
    with open(os.path.join(ROOT_DIR, "abi", "RRoyale.json")) as royaleJson:
        royaleAbi = json.loads(royaleJson.read())['abi']

    #ABI = json.dumps(royaleAbi, separators=(',', ':'))
    def run():
        ...
        #abi = json.loads(ABI)
        abi = royaleAbi
        ROYALE = w3.eth.contract(abi=abi)
        ...
        scanner = EventScanner(
            ...
            contract=ROYALE,
            state=state,
            events=[ROYALE.events[eventName]],
            filters={"address": ROYALE_ADDRESS},
            # How many maximum blocks at the time we request from JSON-RPC
            # and we are unlikely to exceed the response size limit of the JSON-RPC server
            max_chunk_scan_size=10000
        )
        ...
        start_block = max(state.get_last_scanned_block() - chain_reorg_safety_blocks, int(CONTRACT_CREATIONBLOCK))
        end_block = scanner.get_suggested_scan_end_block()
        blocks_to_scan = end_block - start_block
