...
library RoyaleBattleV1 {
    ...
    // ===== Calculated Views ===== 
    // function getUnoccupiedTile(uint256 _roomId, GameRoom storage game) internal view returns (uint8) {
    //     // get random tile
    ...
    // function spawnItems(uint256 _roomId, GameRoom storage game) internal returns (bool) {
    ...
    //     uint8 playersAlive;
    //     for (uint8 i = 0; i < game.playerAlive.length; i++) {
    //         if (game.playerAlive[i] == true) {
    //             playersAlive++;
    //         }
    //     }
    ...
    //     spawnItem(_roomId, 4, game);
    //     spawnItem(_roomId, 5, game);
    //     // spawnItem(_roomId, 6, game);
